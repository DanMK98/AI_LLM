...
class Context64Tests(unittest.TestCase):
    def test_conversion_and_training(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(torch.equal(converted["model_state"]["position_embedding.weight"],
                                       torch.load(repeat, weights_only=True)["model_state"]["position_embedding.weight"]))
            ...
            train(["--resume", str(initial), "--output-dir", str(output), "--steps", "1", "--device", "cpu"])
