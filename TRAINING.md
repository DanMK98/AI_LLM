...
```powershell
.\.venv\Scripts\python.exe train_transformer.py --resume finance_bpe_2048/latest.pt --batch-size 92 --steps 1000 --output-dir finance_bpe_2048
```
...
```powershell
.\.venv\Scripts\python.exe train_transformer.py --scratch --tokenizer finance_bpe_2048/tokenizer.json --batch-size 32 --steps 1000 --eval-every 200 --output-dir finance_bpe_2048
```
...
```powershell
.\.venv\Scripts\python.exe generate_transformer.py --checkpoint finance_bpe_2048/best.pt --prompt "The interest rate" --length 128 --temperature 0.8
.\.venv\Scripts\python.exe train_transformer.py --resume finance_bpe_2048/latest.pt --steps 1000 --output-dir finance_bpe_2048
```
...
resume if input.txt changed; use a new weights-only run for a deliberate data
change. Old checkpoints without fingerprints remain loadable.

Run short temporary-checkpoint tests with:

```powershell
.\.venv\Scripts\python.exe -B -m unittest test_subword -v
```
